...
%niod:Binnenvaart	skos:broader niod:Scheepvaart
%which are to be converted to 
%these two triples:
%[someNumericalURI] skos:broader [someNumericalURI].
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Function broader does exactly this, and writes the results to graph niod.
...
do_broader(S, O):-
    correct_uri(S, NewS),
    correct_uri(O, NewO),
    rdf_assert(NewS, skos:broader, NewO, 'niod').
        

broader:-
   % forall(rdf(S, skos:broader ,O, 'Niod_encoded.ttl'), do_broader(S, O)).    
    forall(rdf(S, skos:broader ,O, 'Test'), do_broader(S, O)).    
